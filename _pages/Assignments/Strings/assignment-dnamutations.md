...
A substring of DNA bases (for example, ACTG) might appear later in the chain in the form of its complement, in reverse.  That is, the complement of ACTG is TGAC, so a string might be represented as ACTG...TGAC (with zero or more additional bases occurring in the middle).  In this example, ACTG is referred to as a sense, and its reversed complement is called an antisense.
